...
create_monitoring_and_tools() {
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
print_info "[plan mode] Would install sysmoni, check-throttled, cursor-guard, kill-cursor, srps-doctor, srps-reload-rules, srps-pull-rules, srps-report"
        return
    fi

    install_bash_sysmoni(){
        local sysmon_path="$1"
        print_info "Installing legacy bash sysmoni..."
...
if [ "${1:-}" = "--help" ]; then
  cat <<'USAGE'
sysmoni - live system monitor
Env flags:
  SRPS_SYSMONI_INTERVAL      Base refresh interval (default 1s)
  SRPS_SYSMONI_PERCORE       1=show per-core bars (default 1)
  SRPS_SYSMONI_FOCUS         Regex to pre-filter top list (optional)
  SRPS_SYSMONI_JSON          1=one-shot JSON snapshot, then exit
  SRPS_SYSMONI_JSON_STREAM   1=NDJSON stream each interval
  SRPS_SYSMONI_JSON_FILE     Path to write JSON (overwrites; streams append)
...
print_footer(){ runtime=$(( $(date +%s) - start_ts )); printf "\n%sTip:%s SRPS_SYSMONI_INTERVAL=2 to slow refresh; SRPS_SYSMONI_FOCUS='node' to focus; Ctrl+C to exit.  Uptime: %ss\n" "$(c 6)" "$(r)" "$runtime"; }
...
EOF
        ...
        print_success "Legacy bash sysmoni installed"
    }
...
# --- sysmoni -----------------------------------------------
    ...
    install_sysmon_go(){
        ...
        if [ -f "go.mod" ] && [ -d "cmd/sysmoni" ]; then
             print_info "Detected local source; building sysmoni-go from current directory..."
             local tmpbin
             tmpbin=$(mktemp)
             if out=$(CGO_ENABLED=0 GOOS=linux GOARCH="$goarch" go build -o "$tmpbin" ./cmd/sysmoni 2>&1); then
                 sudo install -m 0755 "$tmpbin" "$sysmon_go"
                 rm -f "$tmpbin"
                 print_success "Built sysmoni-go from local source"
                 return 0
             else
                 rm -f "$tmpbin"
        ...
        if [ "$build_ref" = "main" ] && command -v curl >/dev/null 2>&1; then
            ...
            if [[ "$latest_url" =~ /releases/tag/(.*)$ ]]; then
                ...
                build_ref="$resolved_tag"
            fi
        fi

        # 1) Try prebuilt release asset
        local bin_url="https://github.com/Dicklesworthstone/system_resource_protection_script/releases/download/${build_ref}/sysmoni-linux-${goarch}"
        local tmpbin
        tmpbin=$(mktemp)
        ...
        print_info "sysmoni-go binary not found at ${bin_url}; will try to build from source"
        rm -f "$tmpbin"
        ...
        if out=$(CGO_ENABLED=0 GOOS=linux GOARCH="$goarch" go build -C "$srcdir" -o "$tmpdir/sysmoni-go" ./cmd/sysmoni 2>&1); then
            sudo install -m 0755 "$tmpdir/sysmoni-go" "$sysmon_go"
            rm -rf "$tmpdir"
            return 0
        fi
        print_warning "Go build of sysmoni-go failed: $out"
        rm -rf "$tmpdir"
        return 1
    }

    if [ "$DRY_RUN" -eq 1 ]; then
        print_info "[plan mode] Would install sysmoni-go binary (or build from source) to $sysmon_go and link $sysmon; no bash fallback in plan"
    else
        if install_sysmon_go "$ref"; then
            sudo ln -sf "$sysmon_go" "$sysmon"
            print_success "sysmoni-go installed and linked to $sysmon"
        else
            print_warning "sysmoni-go download/build failed; falling back to legacy bash sysmoni."
...
complete -W "srps-doctor srps-reload-rules srps-pull-rules srps-report sysmoni check-throttled cursor-guard kill-cursor" srps-doctor srps-reload-rules srps-pull-rules srps-report sysmoni check-throttled cursor-guard kill-cursor
EOF

    print_info "Bash completions installed at $completion_file"

    print_success "Monitoring and helper tools installed (sysmoni, check-throttled, cursor-guard, kill-cursor, srps-doctor, srps-reload-rules${ENABLE_RULE_PULL:+, srps-pull-rules}${ENABLE_HTML_REPORT:+, srps-report})"
}
...
show_final_summary_install() {
    ...
    echo -e "  ${CYAN}sysmoni${NC}         - Live system resource monitor"
...
uninstall_monitoring_tools() {
    ...
    for bin in /usr/local/bin/sysmoni /usr/local/bin/check-throttled /usr/local/bin/cursor-guard /usr/local/bin/kill-cursor /usr/local/bin/srps-doctor /usr/local/bin/srps-reload-rules /usr/local/bin/srps-pull-rules /usr/local/bin/srps-report; do
